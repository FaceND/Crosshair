...
int OnInit()
{
   ...
   if (ShowInfo)
   {
      if(!CreateInfomation(INFO_OPEN,   5)||
         !CreateInfomation(INFO_HIGH,   4)||
         !CreateInfomation(INFO_LOW,    3)||
         !CreateInfomation(INFO_CLOSE,  2)||
         !CreateInfomation(INFO_VOLUME, 1))
      {
         return INIT_FAILED;
      }
      // Create Ask and Bid
      if (ShowBidAsk)
      {
         if (!CreateInfomation(ASK_NAME, 2)||
             !CreateInfomation(BID_NAME, 1))
         {
            return INIT_FAILED;
         }
      }
                ...
                const int             &spread[])
{
   if (ShowInfo)
   {
      ...
      {
         GetCandleInfo(0, current_info);
         UpdateInformation(current_info);
      }
...
//+------------------------------------------------------------------+
bool CreateInfomation(string info_name, int line_number=1)
{
...
void UpdateAskBid(const bool not_blank = true)
{
   if (not_blank)
   {
      ...
      ObjectSetString(0, ASK_NAME, OBJPROP_TEXT, "ask   " + ask);
      ObjectSetString(0, BID_NAME, OBJPROP_TEXT, "bid    " + bid);
   }
   else
   {
...
//+------------------------------------------------------------------+
void UpdateInformation(const string &infoList[], const bool not_blank = true)
{
   if (not_blank)
   {
...
//+------------------------------------------------------------------+
//| Function to move crosshair lines                                 |
//+------------------------------------------------------------------+
void MoveLines(const datetime time, const double price)
{
